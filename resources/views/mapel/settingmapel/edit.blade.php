...
@section('page-title')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 text-gray-800"><a href="{{route('mapelguru.index')}}" class="text-gray-700 mr-4"><i
                    class="fas fa-angle-left"></i></a>Edit Guru Mapel</h1>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item font-weight-normal" aria-current="page">Edit Guru Mapel</li>
        </ol>
    </div>
@endsection
@section('content')
    <div class="card col-lg-6 m-auto">
        <form action="{{route('mapelguru.update',['mapelguru'=>$mapelguru->id])}}" method="post">
            @method('patch')
            @csrf
            <div class="row">
                <!-- Form Sizing -->
                <div class="col-lg-12">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select class="select-single form-control @error('guru_id') is-invalid @enderror"
                                    name="guru_id" id="guru">
                                <option value="{{null}}">Select..</option>
                                @foreach($guru as $data)
                                        <option @if($data->id == $mapelguru->id) selected @endif value="{{$data->id}}">{{$data->nama}}</option>
                                @endforeach
                            </select>
                            @error('guru_id')
                        ...
                        <div class="form-group">
                            ...
                            <select name="mapel[]" multiple="multiple" id="myMultiple"
                                    class="select2-multiple form-control @error('mapel') is-invalid @enderror">
