            ...
            <div class="card">
                <div class="p-3 mx-2 mt-2">
                    <form class="form-horizontal form-material"
                          action="{{route('walimurid.update',['walimurid'=>$walimurid->id])}}"
                          method="POST">
                        @csrf
                        @method('PATCH')
                        <div class="form-group">
                            ...
                            <div class="col-md-12">
                                <input required type="text" name="nama" value="{{$walimurid->nama}}"
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-12 input-group flex-nowrap">
                                ...
                                {{--                                </div>--}}
                                <input type="text" required name="nohp" value="{{$walimurid->nohp}}"
                                       class="form-control form-control-line">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-12">
                                <select name="gender" class="form-control">
                                    @foreach($kelamin as $data)
                                        <option @if($data->jeniskelamin == $walimurid->gender->jeniskelamin) selected
                                                @endif value="{{$data->id}}">{{$data->jeniskelamin}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                                ...
                                <select name="agama" class="form-control">
                                    @foreach($agama as $data)
                                        @if($data->agama== $walimurid->agama->agama)
                                            <option selected
                                                    value="{{$walimurid->agama->id}}">{{$walimurid->agama->agama}}</option>
                                        @else
                                ...
                                <select name="pekerjaan" class="form-control">
                                    @foreach($pekerjaan as $data)
                                        @if($data->pekerjaan == $walimurid->pekerjaan->pekerjaan)
                                            <option selected
                                                    value="{{$walimurid->pekerjaan->id}}">{{$walimurid->pekerjaan->pekerjaan}}</option>
                                        @else
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-12">
                                <input required type="text" name="alamat" value="{{$walimurid->alamat}}"
                                       class="form-control form-control-line">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-12">
                                @can('walimurid.update')
                                    <button type="submit" class="btn btn-success">Update Profile</button>
                                @endcan
                                @can('walimurid.delete')
                    ...
                    @can('walimurid.delete')
                        <form id="deleteform" method="POST"
                              action="{{route('walimurid.destroy',['walimurid'=>$walimurid->id])}}">
                            @csrf
                            @method('DELETE')
                        </form>
                    @endcan
