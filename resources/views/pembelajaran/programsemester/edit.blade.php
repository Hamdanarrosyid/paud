...
@section('content')
    ...
    <div>
        ...
        <div class="card-body">
            <form action="{{route('program-semester.update',['program_semester'=>$programSemester->id])}}" method="post">
                @csrf
                @method('PATCH')
                <div class="form-group row">
                    ...
                    <select required name="semester_id" id="inputSemester"
                        ...
                        @foreach($semester as $data)
                            <option
                                @if(old('semester_id') == $data->id || $programSemester->semester_id == $data->id)
                                selected
                                @endif
                ...
                <div class="form-group row">
                    ...
                    <select required name="tema_id" id="inputTema"
                        ...
                        @foreach($tema as $data)
                            <option
                                @if(old('tema_id') == $data->id || $programSemester->tema_id == $data->id)
                                selected
                                @endif
                    ...
                    <select required name="sub_tema_id[]" multiple="multiple" id="sub_tema_select"
                        ...
                        @foreach($subtema as $data)
                            <option
                                ...
                                @endif
                                @endforeach
                                @elseif($programSemester->tema->subTema == !null)
                                @foreach($programSemester->tema->subTema as $sub_tema)
                                @if($sub_tema->id == $data->id)
                                selected
                                @endif
                                @endforeach
                                @endif
                    ...
                    <select required name="kompetensi_dasar_id[]" multiple="multiple" id="kd_select"
                        ...
                        @foreach($kd as $data)
                            <option
                                ...
                                @endif
                                @endforeach
                                @elseif($programSemester->tema->kd == !null)
                ...
                <div class="form-group row">
                    ...
                    <select required name="bulan_id" id="inputWaktu"
                        ...
                        @foreach($bulan as $data)
                            <option
                                @if(old('bulan_id') == $data->id || $programSemester->bulan_id == $data->id)
                                selected
                                @endif
